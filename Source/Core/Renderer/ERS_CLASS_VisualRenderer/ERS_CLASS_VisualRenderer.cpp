...
void ERS_CLASS_VisualRenderer::UpdateViewport(int Index, ERS_CLASS_SceneManager* SceneManager, float DeltaTime, bool DrawCursor) {
    ...
    if (Visible) {
        ...
        if (SceneManager->Scenes_[SceneManager->ActiveScene_]->HasSelectionChanged && DrawCursor) {
            ...
            // Get LocRotScale
            glm::vec3 Position = SceneManager->Scenes_[SceneManager->ActiveScene_]->ModelsWithoutTransparency_[SelectedModel]->ModelPosition;        
            glm::vec3 Rotation = SceneManager->Scenes_[SceneManager->ActiveScene_]->ModelsWithoutTransparency_[SelectedModel]->ModelRotation;        
            glm::vec3 Scale = SceneManager->Scenes_[SceneManager->ActiveScene_]->ModelsWithoutTransparency_[SelectedModel]->ModelScale;
...
void ERS_CLASS_VisualRenderer::CreateViewport(std::string ViewportName) {
    ...
    Viewport->Grid = std::make_unique<ERS_CLASS_Grid>(SystemUtils_, Shaders_[0]);
    Viewport->LightIconRenderer = std::make_unique<ERS_CLASS_LightIconRenderer>(OpenGLDefaults_, SystemUtils_, Shaders_[1]); //Set TO Shader 19 For Billboard Shader, Temp. Disabled As It Doesn't Work ATM
