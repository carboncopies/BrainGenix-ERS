...
void ERS_CLASS_VisualRenderer::CreateViewport(std::string ViewportName) {
    ...
    Viewport->Grid = std::make_unique<ERS_CLASS_Grid>(SystemUtils_, Shaders_[ERS_FUNCTION_FindShaderByName(std::string("_Grid").get(), &Shaders_)]);
    Viewport->LightIconRenderer = std::make_unique<ERS_CLASS_LightIconRenderer>(OpenGLDefaults_, SystemUtils_, Shaders_[ERS_FUNCTION_FindShaderByName(std::string("_LightIcon").get(), &Shaders_)]); //Set TO Shader 19 For Billboard Shader, Temp. Disabled As It Doesn't Work ATM
