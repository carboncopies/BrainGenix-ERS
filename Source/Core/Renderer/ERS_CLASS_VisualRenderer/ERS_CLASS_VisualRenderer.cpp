...
void ERS_CLASS_VisualRenderer::UpdateShader(int ShaderIndex, float DeltaTime, int RenderWidth, int RenderHeight, ERS_CLASS_SceneManager*SceneManager, ERS_STRUCT_Camera* Camera) {
    ...
    **/
    ...
    for (int i = 0; i < NumberSpotLights; i++) {
        ...
        ActiveShader->SetFloat((UniformName + std::string(".CutOff")).c_str(), 1.0f - (ActiveScene->SpotLights[i]->CutOff * (0.01745329 / 2)));
        ActiveShader->SetFloat((UniformName + std::string(".RollOff")).c_str(), (ActiveScene->SpotLights[i]->Rolloff * (0.01745329 / 2)));
