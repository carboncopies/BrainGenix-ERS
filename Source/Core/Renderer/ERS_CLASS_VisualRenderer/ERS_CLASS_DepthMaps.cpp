...
void ERS_CLASS_DepthMaps::UpdateDepthMap(ERS_STRUCT_DepthMap* Target, ERS_STRUCT_Shader* DepthShader, glm::vec3 Pos, bool Orthogonal, glm::mat4* LightSpaceMatrix) {
...
void ERS_CLASS_DepthMaps::UpdateDepthMaps(ERS_STRUCT_Shader* DepthShader) {
    ...
    for (unsigned int i = 0; i < ActiveScene->PointLights.size(); i++) {
        ...
        // Render To Depth Map
        UpdateDepthMap(&Light->DepthMap, DepthShader, Light->Pos, false); // set this to false later, debugging
