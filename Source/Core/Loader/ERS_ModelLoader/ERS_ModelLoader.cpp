...
void ERS_CLASS_ModelLoader::ProcessGPU(std::shared_ptr<ERS_STRUCT_Model> Model) {
    ...
    for (int i = 0; (long)i < (long)Model->TexturesToPushToGPU_.size(); i++) {
        ...
        if (RawImageData != NULL) {
            ...
            Model->TextureColorChannels_.push_back(Model->TexturesToPushToGPU_[i].Channels);            
...
ERS_STRUCT_Mesh ERS_CLASS_ModelLoader::ProcessMesh(aiMesh *Mesh, const aiScene *Scene, std::vector<std::string> TexturePaths) {
...
void ERS_CLASS_ModelLoader::LoadMaterialTextures(std::vector<int>* IDs, std::vector<std::string>* Types, std::vector<std::string> TextureList, aiMaterial *Mat, aiTextureType Type, std::string TypeName) {
    ...
    for (unsigned int i=0; i< Mat->GetTextureCount(Type); i++) {
        ...
        int Index = -1;
        for (int x = 0; (long)x < (long)TextureList.size(); x++) {
