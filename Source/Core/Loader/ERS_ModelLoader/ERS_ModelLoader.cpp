        ...
        if (RawImageData != NULL) {
            if (Model->TexturesToPushToGPU_[i].FreeImageBackend) {
                ...
                if (Model->TexturesToPushToGPU_[i].Channels == 4) {
                    ...
                    std::cout<<"GLGenTex Says: "<<glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, Model->TexturesToPushToGPU_[i].Width, Model->TexturesToPushToGPU_[i].Height, 0, GL_BGRA, GL_UNSIGNED_BYTE, RawImageData);
                } else if (Model->TexturesToPushToGPU_[i].Channels == 3) {
                    std::cout << "3\n";

                    std::cout<<"GLGenTex Says: "<<glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, Model->TexturesToPushToGPU_[i].Width, Model->TexturesToPushToGPU_[i].Height, 0, GL_BGR, GL_UNSIGNED_BYTE, RawImageData);
                } else if (Model->TexturesToPushToGPU_[i].Channels == 2) {
                    std::cout << "4\n";

                    std::cout<<"GLGenTex Says: "<<glTexImage2D(GL_TEXTURE_2D, 0, GL_RG, Model->TexturesToPushToGPU_[i].Width, Model->TexturesToPushToGPU_[i].Height, 0, GL_RG, GL_UNSIGNED_BYTE, RawImageData);
                } else if (Model->TexturesToPushToGPU_[i].Channels == 1) {
                    std::cout << "4\n";

                    std::cout<<"GLGenTex Says: "<<glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, Model->TexturesToPushToGPU_[i].Width, Model->TexturesToPushToGPU_[i].Height, 0, GL_RED, GL_UNSIGNED_BYTE, RawImageData);
                } else {
                    std::cout << "5\n";
