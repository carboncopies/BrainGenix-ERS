...
bool ERS_FUNCTION_DecodeSceneV2(YAML::Node SceneData, ERS_STRUCT_Scene *Scene, ERS_STRUCT_SystemUtils *SystemUtils, ERS_CLASS_ModelLoader* ModelLoader, bool LogEnable) {
    ...
    for (long i = 0; (long)i < (long)SceneDataNode.size(); i++) {
        ...
        if (AssetType == std::string("Model")) {
            ...
            Scene->SceneCameras[SceneCameraIndex]->EnforceAspectRatio_      = SceneDataNode[i]["EnforceAspectRatio"].as<bool>();
            Scene->SceneCameras[SceneCameraIndex]->AspectRatio_             = SceneDataNode[i]["AspectRatio"].as<float>();
            Scene->SceneCameras[SceneCameraIndex]->StreamingPriority_       = SceneDataNode[i]["StreamingPriority"].as<int>();
